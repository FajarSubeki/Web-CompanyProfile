...
class Pemanfaatan_model extends CI_Model {
    ...
    public $default_table = 't_pemanfaatan';
   ...
   function count_by($id_user) {
        $this->db->select('*');
        $this->db->from('t_pemanfaatan');
        $this->db->join('t_pemohon', 't_pemohon.id_pemohon = t_pemanfaatan.id_pemohon');        
   ...
   function get_all_admin_proses() {
    ...
    function get_all_admin_hasil() {
        ...
        $this->db->where('t_pemohon.status','4'); 
        $this->db->order_by("t_pemohon.id_pemohon","desc");
    ...
    function get_by_id_pemanfaatan($id)
    {
